# ui/app.py
"""
Streamlit UI for auto-cleaner-pro
...
# Add local src so the app can import your package during development
import sys
sys.path.insert(0, str(Path.cwd() / "src"))

from auto_cleaner_pro.cleaning import load_csv_auto, clean_dataframe

# ---- Page config and simple style -----------------------------------------
...
if uploaded is not None:
    ...
    if example_path.exists():
        ...
        # read bytes so we can feed into loader (it expects a path; we'll write a temp file)
        input_bytes = example_path.read_bytes()
    else:
        st.sidebar.error("Example file not found in examples/test_messy.csv")

# ---- Main action: Clean button -------------------------------------------
if st.button("Clean"):
    ...
    if input_source is None:
        ...
        # If uploaded or example, write to a temp path inside .st_cache for loader convenience
        ...
        with col1:
            ...
            st.dataframe(df_raw.head(200))  # show up to 200 rows in the UI
        with col2:
        ...
        # ---- Download cleaned CSV + report --------------------------------
        # Prepare cleaned CSV bytes
        ...
        # ---- Optionally write to examples/ if not dry_run and not report_only ----
...
# ---- Footer / credits -----------------------------------------------------
st.markdown("---")
st.markdown("Built with ❤️ using Streamlit — `auto-cleaner-pro`")
